<template>
     <div class="px-6">
        <h1 class="text-4xl px-4 font-bold text-white my-3 border-l-4 border-secondary rounded-s">Edit Book</h1>

        <form @submit.prevent="handleSubmit">
            <label class="input input-bordered flex items-center gap-2 my-4 text-white font-bold">
                Title
                <input type="text" class="grow" :placeholder=titleBook v-model="books.title"/>
            </label>
            ...
            <label class="grid items-center gap-2 my-4">
                ...
                <textarea class="textarea textarea-bordered" rows="3"  :placeholder=summaryBook v-model="books.summary"></textarea>
            </label>

            <label class="input input-bordered flex items-center gap-2 my-4 font-bold text-white">
                Stok
                <input type="text" class="grow"  :placeholder=stokBook v-model="books.stok"/>
            </label>
            ...
            <input type="submit" value="Edit" class="btn btn-secondary btn-block">
        </form>
     </div>
</template>
...
const handleSubmit = async() => {
    try {
        ...
        const editBook = await customAPI.post(`/books/${route.params.id}?_method=PUT`, formData, {headers: {Authorization: `Bearer ${authStore.tokenUser}`}});
...
const titleBook = ref("");
const summaryBook = ref("");
const stokBook = ref("");

const BookById = async() => {
    const { data } = await customAPI(`/books/${route.params.id}`);
    titleBook.value = data.data.title;
    summaryBook.value = data.data.summary;
    stokBook.value = data.data.stok;
}
